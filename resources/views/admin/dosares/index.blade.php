...
@can('dosare_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.dosares.create') }}">
                ...
                <tbody>
                    @foreach($dosares as $key => $dosare)
                        <tr data-entry-id="{{ $dosare->id }}">
                            ...
                            <td>
                                @can('dosare_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.dosares.show', $dosare->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('dosare_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.dosares.edit', $dosare->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('dosare_delete')
                                    <form action="{{ route('admin.dosares.destroy', $dosare->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('dosare_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.dosares.massDestroy') }}",
